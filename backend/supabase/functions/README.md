...
4. Deploy: `supabase functions deploy gemini-assessment --no-verify-jwt`.
...
`--no-verify-jwt` supports the current unauthenticated kiosk flow. Before a public launch, add authentication and rate limiting, then enable JWT verification. The output is decision support only; it is not a diagnosis or prescription.
